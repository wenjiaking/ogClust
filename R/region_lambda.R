#' @title efficient lambda grids for a given K, for continuous outcome
...
region_lambda<-function (lambda1 =18,lambda2=0,iteration = 10, Y, G,X,center,w,K){
  ...
  for (i in 1:length(num.vector)) {
    mod1 <- ogClust_WJL(x=X,G=t(G),y=Y,c_center=center,lambda=lambda.vector[i],v_int=NULL,pi_int=NULL,K=K,max_iter=200,w_outcome=w,w_G=1-w,z_int=NULL)
  ...
  while (iter <= iteration) {
    ...
    mod_new<-ogClust_WJL(x=X,G=t(G),y=Y,c_center=center,lambda=lambda,v_int=NULL,pi_int=NULL,K=K,max_iter=200,w_outcome=w,w_G=1-w,z_int=NULL)

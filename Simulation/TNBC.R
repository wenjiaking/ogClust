...
wcs.GM<-mclapply(1:length(lambda_vector_Peng),function(ind_lambda){
  ...
  mod.all<-fit.ogClust.surv(n=n1, K=K, np=np, NG=NG, lambda=lambda,delta = delta.train,
                            alpha=0.5, G=X.train, Y=Y.train, X=as.data.frame(Z.train), theta_int=theta_int)
...
mod.GM<-fit.ogClust.surv(n=n1, K=K, np=np, NG=NG, lambda=lambda,delta = delta.train,
                                 alpha=0.5, G=as.data.frame(X.train), Y=Y.train, X=as.data.frame(Z.train), theta_int=theta_int)
########## SKM ######################
K=2
set.seed(12315)
...
#########Generate figures##########
readIN.survEXT=function(sc=TRUE,ngenes=100,tune_time=tuneweight.SC.res$tune_time) {
  ...
  if (sc) {
    load(paste0("survmod1.SC.WJL.fix",ngenes,".RData"))
    load(paste0("survmod.SC.all.fix",ngenes,".RData"))
    ...
    load(paste0("survmod1.WJL.fix",ngenes,".RData"))
...
plotlist=readIN.survEXT(sc=TRUE, ngenes=400,tune_time = tuneweight.SC.res$tune_time)
...
plot.R2_genes<-ggarrange(plotlist[[9]],plotlist[[10]],nrow=2)

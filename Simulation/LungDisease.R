...
#######Tune parameter for GM ######

set.seed(12315)
K=3
lambda_vector1=seq(0.0001,0.015,0.0005)
...
wcs.GM<-mclapply(1:length(lambda_vector1),function(ind_lambda){
  ...
  mod.all<-fit.ogClust(n=n1, K=K, np=np, NG=NG, lambda=lambda,
                       alpha=1, G=X.GSE47460_1, Y=Y1, X=Z.GSE47460_1, theta_int=theta_int)
...
######Fixed number of genes for GM, SKM, and PMBC ########
fixgenes.fit=function(X=X.GSE47460_1,Y=Y1,Z=Z.GSE47460_1,K=3,lambda.GM,wbound,lambda.PMBC) {
  ...
  mod.ogClust<-fit.ogClust(n=n1, K=K, np=np, NG=NG, lambda=lambda.GM,
                           alpha=1, G=as.data.frame(X), Y=Y, X=Z, theta_int=theta_int)
  end_time <- Sys.time()
...
# For GM model:
# lambda<-0.001  select about 300 genes
# lambda<- 0.0003  select about 400 genes
...
set.seed(12315)
start_time <- Sys.time()
modfit=fixgenes.fit(lambda.GM=0.0003,wbound=16.5,lambda.PMBC=94.28571)
end_time <- Sys.time()
save(modfit,file="mod.all.fix400.Rdata")
set.seed(12315)
start_time <- Sys.time()
modfit=fixgenes.fit(lambda.GM=0.001,wbound=14.2,lambda.PMBC=101)
end_time <- Sys.time()

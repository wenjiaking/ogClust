...
library(tidyr)
library(ggplot2)
summary_gen=function(SC=TRUE,tune=FALSE,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2),res.file="SimI_RES",figure.name="SupplementFigrueI.png"){
  ...
  for(ind_c1 in 1:length(c1_vector_all)) {
    ...
    for (ind_mu in 1:length(mu_vector)) {
      mu=mu_vector[ind_mu]
      if (SC) {
        file.name= paste(res.file,"_c1=",c1,"_mu=",mu,".SC.rds",sep="")
      }
      else {
        file.name= paste(res.file,"_c1=",c1,"_mu=",mu,".rds",sep="")
      }
...
#generate Figrue I
summary_gen(SC=FALSE,tune=TRUE,c1_vector_all=c(2),figure.name = "FigureI.png")
#generate Supplementary Figrue II
summary_gen(SC=FALSE,tune=TRUE,c1_vector_all=c(3),figure.name="SupplementFigrueIII.png")
#generate Supplementary Figrue III
summary_gen(SC=FALSE,tune=FALSE,c1_vector_all=c(2),figure.name="SupplementFigrueIV.png")
#generate Supplementary Figrue IV
summary_gen(SC=TRUE,tune=TRUE,c1_vector_all=c(2),figure.name="SupplementFigrueV.png")

#########GM with penalized LDA ##########3
resLDA.gen=function(K=3,SC=TRUE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2,1)) {
  ...
  for(ind_c1 in 1:length(c1_vector_all)) {
    ...
    for (ind_mu in 1:length(mu_vector)) {
      ...
      RES=mclapply(1:num.sim,function(ind.data) {
        ...
        #------------------------------
        if (SC) {
          lambda_vector_Peng<-seq(0.07,0.1,0.001)
        }
        else {
          lambda_vector_Peng<-seq(0.05,0.1,0.002)
        }
        #lambda_vector_Peng<-c(seq(0.005,0.01,0.001),seq(0.02,0.5,0.02))
        np=ncol(X.train)
        R2.outcome.tune<-rep(NA,length(lambda_vector_Peng))
        ...
        for(ind_lambda in 1:length(lambda_vector_Peng)){
          #print(i)
          fit.res<-fit.ogClust.LDA(K=K,lambda=lambda_vector_Peng[ind_lambda],G=G.train,Y=Y.train, X=X.train)

          LDAout <- PenalizedLDA(G.train,fit.res$label,lambda=fit.res$lambda,K=(K-1))
          select.feature=as.numeric(apply(LDAout$discrim,1,function(x) any(x!=0)))
          num[ind_lambda]<-sum(select.feature==1)
          print(num)
          cluster1<-fit.res$grp_assign
          ...
          for(ind_cv in 1:length(unique(index.sample))){
            ...
            mod<-fit.ogClust.LDA(K=K,lambda=lambda_vector_Peng[ind_lambda],G=G.train1,Y=Y.train1, X=X.train1)

            out.test <- PenalizedLDA(G.train1,mod$label,xte=G.test1,lambda=mod$lambda,K=(K-1))
        ...
        fit.res.tune<-fit.ogClust.LDA(K=K,lambda=lambda,G=G.train,Y=Y.train, X=X.train)

        out.test <- PenalizedLDA(G.train,fit.res.tune$label,xte=G.test,lambda=fit.res.tune$lambda,K=(K-1))
        select.feature=as.numeric(apply(out.test$discrim,1,function(x) any(x!=0)))
        cluster.test=out.test$ypred[,(K-1)]
        ...
        beta_est=fit.res.tune$par[1:np]
        beta0_est=fit.res.tune$par[(np + 1):(np + K)]
        sigma2_est=fit.res.tune$par[(np + K + 1)]
        ...
        print(paste0("Tuning lambda=",lambda.num," that selects  ",num[index]," features."))
        start_time=Sys.time()
        fit.res.num<-fit.ogClust.LDA(K=K,lambda=lambda.num,G=G.train,Y=Y.train, X=X.train)
        end_time=Sys.time()
        time.GM=as_hms(end_time-start_time)

        out.test <- PenalizedLDA(G.train,fit.res.num$label,xte=G.test,lambda=fit.res.num$lambda,K=(K-1))
        select.feature=as.numeric(apply(out.test$discrim,1,function(x) any(x!=0)))
        cluster.test=out.test$ypred[,(K-1)]
        ...
        beta_est=fit.res.num$par[1:np]
        beta0_est=fit.res.num$par[(np + 1):(np + K)]
        sigma2_est=fit.res.num$par[(np + K + 1)]
      ...
      if (SC) {
        file.res<-paste("SimI_RES_c1=",c1,"_mu=",mu,".SC.rds",sep="")
        file.update<-paste("SimI_RES_GMupdated_c1=",c1,"_mu=",mu,".SC.rds",sep="")
      }
      else {
        file.res<-paste("SimI_RES_c1=",c1,"_mu=",mu,".rds",sep="")
        file.update<-paste("SimI_RES_GMupdated_c1=",c1,"_mu=",mu,".rds",sep="")
      }
      RES.old=readRDS(file.res)
      RES.update=lapply(which(sapply(RES.old,length)==5),function(i) {
        RES.old[[i]]$GM.res=RES[[i]]
        return(RES.old[[i]])
      })
      saveRDS(RES.update,file.update)
      # saveRDS(RES,file.res)
    }
  }
}

res=resLDA.gen(K=3,SC=FALSE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3))
#generate Supplementary Figrue VIII
summary_gen(SC=FALSE,tune=TRUE,c1_vector_all=c(3),res.file = "SimI_RES_GMupdated",figure.name="SupplementFigrueVIII.png")

####Generate simulated data (unbalanced cluster)####
mu_vector<-c(0.9,1.2,1.5,1.8)
mu1_vector<-mu_vector+0.2
c1_vector_all<-c(3,2,1)
beta_vector<-2
...
for(ind_c1 in 1:length(c1_vector_all)) {
  ...
  for (ind_mu in 1:length(mu_vector)) {
    ...
    mu1<-mu1_vector[ind_mu]
    #file.name<-paste("SimI_Data_c1=",c1,"_mu=",mu,".rds",sep="")
    file.name<-paste("SimUnbalance",round(frac3,2),"_Data_c1=",c1,"_mu=",mu,".rds",sep="")
    data.temp=mclapply(1:num.sim,function(ind.data) {
      set.seed(ind.data)
      data<-SimUnbalance(n=n,beta1=beta_vector,beta2=beta_vector,q=q,q1=q1,q2=q2,q3=q3,c1=c1,var_g=var_g,mu=mu,mu1=mu1,sigma_y=sigma_y,frac3=frac3)
      X<-data$x
      Y<-data$y
      G<-data$G
      n1=floor(n*((1-frac3)/2))
      n3=n-2*n1
      true.label<-c(rep(1,n1),rep(2,n1),rep(3,n3))
      true.feature<-c(rep(1,q),rep(0,2000-q))
      return(list(X=X,Y=Y,G=G,true.label=true.label,true.feature=true.feature))
    },mc.cores=num.sim)
    saveRDS(data.temp,file.name)
...
resUnbalance.gen=function(K=3,SC=TRUE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2,1),frac3=1/5) {
  ...
  for(ind_c1 in 1:length(c1_vector_all)) {
    ...
    for (ind_mu in 1:length(mu_vector)) {
      ...
      RES=mclapply(1:num.sim,function(ind.data) {
        ...
        cluster.test.tune=cluster.test
        cluster.train.tune=cluster.train
        #---------------------------------------------
        #use num to select lambda
        #---------------------------------------------
        index<-max(which(num.vector>=50))
        ...
        cluster.test.num=cluster.test
        cluster.train.num=cluster.train
        #-------------------------------------------------
        WJL.res<-list(mod.tune=mod.tune,mod.num=mod.num,
                      pred.train.tune=res.train.tune,pred.train.num=res.train,
                      pred.test.tune=res.test.tune,pred.test.num=res.test,
                      ...
                      ari.consistent.train=ari.consistent.WJL,
                      ari.consistent.test=ari.consistent.test.WJL,
                      cluster.test.tune=cluster.test.tune,cluster.train.tune=cluster.train.tune,
                      cluster.test.num=cluster.test.num,cluster.train.num=cluster.train.num)
        print("Finish WJL method!")
        #------------------------------
        #Peng's method
        #------------------------------
        ...
        #lambda_vector_Peng<-c(seq(0.005,0.01,0.001),seq(0.02,0.5,0.02)) # for elastic net
        lambda_vector_Peng<-seq(0.001,0.1,0.005)
        ...
        for(ind_lambda in 1:length(lambda_vector_Peng)){
          #print(i)
          fit.res<-fit.ogClust(n=n1, K=K, np=np, NG=NG, lambda=lambda_vector_Peng[ind_lambda],alpha=1, G=as.data.frame(G.train), Y=Y.train, X=X.train, theta_int=theta_int)
          ...
          for(ind_cv in 1:length(unique(index.sample))){
            ...
            np=ncol(X.train1)
            mod<-fit.ogClust(n=n1sub, K=K, np=np, NG=NGsub, lambda=lambda_vector_Peng[ind_lambda],
                             alpha=1, G=as.data.frame(G.train1), Y=Y.train1, X=X.train1, theta_int=theta_int)
        ...
        fit.res.tune<-fit.ogClust(n=n1, K=K, np=np, NG=NG, lambda=lambda,
                                  alpha=1, G=as.data.frame(G.train), Y=Y.train, X=X.train, theta_int=theta_int)
        ...
        Rmse.test.Peng.tune<-sqrt(sum((y.pred.test-Y.test)^2)/length(Y.test))
        ari.test.Peng.tune<-adjustedRandIndex(cluster.test,label.test)
        cluster.train.tune=fit.res.tune$grp_assign
        cluster.test.tune=cluster.test
        #---------------------------------------------
        #use num to select lambda
        #---------------------------------------------
        ...
        cluster.train.num=fit.res.num$grp_assign
        cluster.test.num=cluster.test
        #-------------------------------------------------
        GM.res<-list(mod.tune=fit.res.tune,mod.num=fit.res.num,
                     ari.train.tune=ari.train.Peng.tune,ari.train.num=ari.train.Peng.num,
        ...
        cluster.train.tune=mod.tune[[1]]$Cs
        cluster.test.tune=cluster.test
        ...
        cluster.train.num=mod.num$Cs
        cluster.test.num=cluster.test.num
        ...
        PMBC.res<-list(mod.tune=mod.tune,mod.num=mod.num,
                       ...
                       cluster.test.num=cluster.test.num,cluster.train.num=cluster.train.num)
        #return(list(WJL.res=WJL.res,GM.res=GM.res,SKM.res=SKM.res,PMBC.res=PMBC.res,index.test=index.test))
        return(list(WJL.res=WJL.res,GM.res=GM.res,SKM.res=SKM.res,PMBC.res=PMBC.res,
                    index.test=index.test,label.test=label.test,label.train=label.train))
      },mc.cores=150)
      if (SC) {
        file.res<-paste("SimI_RES_c1=",c1,"_mu=",mu,".SC.rds",sep="")
      }
        ...
        #file.res<-paste("SimI_RES_c1=",c1,"_mu=",mu,".rds",sep="")
        file.res<-paste("SimUnbalance",round(frac3,2),"_RES_c1=",c1,"_mu=",mu,".rds",sep="")
      }
      # RES.old=readRDS(file.res)
      # RES.update=lapply(which(sapply(RES.old,length)==4),function(i) {
      #   RES.old[[i]]$GM.res=RES[[i]]
      #   return(RES.old[[i]])
      # })
      # saveRDS(RES.update,file.res)
      saveRDS(RES,file.res)
    }
  }
}

resUnbalanced=resUnbalance.gen(K=3,SC=FALSE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2),frac3=1/5)
resUnbalanced=resUnbalance.gen(K=3,SC=FALSE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2),frac3=1/7)
resUnbalanced=resUnbalance.gen(K=3,SC=FALSE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2),frac3=1/4)
resUnbalanced=resUnbalance.gen(K=3,SC=FALSE,num.sim=50,mu_vector=c(0.9,1.2,1.5,1.8),c1_vector_all=c(3,2),frac3=1/3)

#######Unbalanced Summary ##########
confmat=function(truth,report,C=3) {
  mat=table(truth,report)
  C.ind=which.max(mat[,C])
  sensit=mat[C.ind,C]/sum(mat[,C])
  precis=mat[C.ind,C]/sum(mat[C.ind,])
  return(c(sensitivity=sensit,precision=precis))
}
summaryUnbalance_gen=function(SC=TRUE,tune=FALSE,frac3_vector=c(1/3,1/4,1/5,1/7),c1_vector_all=c(3,2,1),C=3,mu=0.9,figure.name="SupplementFigrueI.png"){

  ari.train.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  sensit3.train.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  # precis1.train.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(mu_vector)))
  # precis2.train.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(mu_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(mu_vector)))
  precis3.train.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  # precis1.test.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(mu_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(mu_vector)))
  # precis2.test.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(mu_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(mu_vector)))
  precis3.test.yujia.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.yujia.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.Peng.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.Peng.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))


  ari.train.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.skmeans.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.skmeans.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.PMBC.matrix.num<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  ari.train.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  ari.test.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.train.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  sensit3.test.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.train.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))
  precis3.test.PMBC.matrix.num.se<-matrix(NA,nrow=length(c1_vector_all),ncol=length(frac3_vector),dimnames = list(paste("c1=",c1_vector_all,sep=""),1:length(frac3_vector)))

  for(ind_c1 in 1:length(c1_vector_all)) {
    c1<-c1_vector_all[ind_c1]
    for (ind_mu in 1:length(frac3_vector)) {
      frac3=frac3_vector[ind_mu]
      if (SC) {
        file.name= paste("SimI_RES_GMlasso_c1=",c1,"_mu=",mu,".SC.rds",sep="")
      }
      else {
        #file.name= paste("SimI_RES_GMlasso_c1=",c1,"_mu=",mu,".rds",sep="")
        #file.name= paste("SimUnbalance_RES_c1=",c1,"_mu=",mu,".rds",sep="")
        file.name<-paste("SimUnbalance",round(frac3,2),"_RES_c1=",c1,"_mu=",mu,".rds",sep="")
      }
      RES=readRDS(file.name)
      #RES=RES[sapply(RES,length)==5]
      RES=RES[sapply(RES,length)==7]
      label.train.true=sapply(RES,"[[",7)
      label.test.true=sapply(RES,"[[",6)
      print(file.name)
      if (tune) {
        ari.train.yujia.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$WJL.res$ari.train.tune})),na.rm=T)
        ari.test.yujia.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$WJL.res$ari.test.tune})),na.rm=T)

        # confmat1.train.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.train.tune,label.train.true[,x],1)})
        # confmat2.train.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.train.tune,label.train.true[,x],2)})
        confmat3.train.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.train.tune,label.train.true[,x],C)})
        # confmat1.test.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.test.tune,label.test.true[,x],1)})
        # confmat2.test.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.test.tune,label.test.true[,x],2)})
        confmat3.test.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.test.tune,label.test.true[,x],C)})
        # sensit1.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat1.train.yujia,"[[",1))
        # precis1.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat1.train.yujia,"[[",2))
        # sensit2.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat2.train.yujia,"[[",1))
        # precis2.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat2.train.yujia,"[[",2))
        sensit3.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.yujia,"[[",1))
        precis3.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.yujia,"[[",2))
        # sensit1.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat1.test.yujia,"[[",1))
        # precis1.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat1.test.yujia,"[[",2))
        # sensit2.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat2.test.yujia,"[[",1))
        # precis2.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat2.test.yujia,"[[",2))
        sensit3.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.yujia,"[[",1))
        precis3.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.yujia,"[[",2))

        ari.train.yujia.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$WJL.res$ari.train.tune})),na.rm=T)
        ari.test.yujia.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$WJL.res$ari.test.tune})),na.rm=T)
        # sensit1.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat1.train.yujia,"[[",1))
        # precis1.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat1.train.yujia,"[[",2))
        # sensit2.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat2.train.yujia,"[[",1))
        # precis2.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat2.train.yujia,"[[",2))
        sensit3.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.yujia,"[[",1))
        precis3.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.yujia,"[[",2))
        # sensit1.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat1.test.yujia,"[[",1))
        # precis1.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat1.test.yujia,"[[",2))
        # sensit2.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat2.test.yujia,"[[",1))
        # precis2.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat2.test.yujia,"[[",2))
        sensit3.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.yujia,"[[",1))
        precis3.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.yujia,"[[",2))

        ari.train.Peng.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$GM.res$ari.train.tune})),na.rm=T)
        ari.test.Peng.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$GM.res$ari.test.tune})),na.rm=T)

        confmat3.train.Peng <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$GM.res$cluster.train.tune,label.train.true[,x],C)})
        confmat3.test.Peng <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$GM.res$cluster.test.tune,label.test.true[,x],C)})
        sensit3.train.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.Peng,"[[",1))
        precis3.train.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.Peng,"[[",2))
        sensit3.test.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.Peng,"[[",1))
        precis3.test.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.Peng,"[[",2))

        ari.train.Peng.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$GM.res$ari.train.tune})),na.rm=T)
        ari.test.Peng.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$GM.res$ari.test.tune})),na.rm=T)

        sensit3.train.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.Peng,"[[",1))
        precis3.train.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.Peng,"[[",2))
        sensit3.test.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.Peng,"[[",1))
        precis3.test.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.Peng,"[[",2))

        ari.train.skmeans.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$SKM.res$ari.train.tune})),na.rm=T)
        ari.test.skmeans.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$SKM.res$ari.test.tune})),na.rm=T)

        confmat3.train.skmeans <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$SKM.res$cluster.train.tune,label.train.true[,x],C)})
        confmat3.test.skmeans <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$SKM.res$cluster.test.tune,label.test.true[,x],C)})
        sensit3.train.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.skmeans,"[[",1))
        precis3.train.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.skmeans,"[[",2))
        sensit3.test.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.skmeans,"[[",1))
        precis3.test.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.skmeans,"[[",2))

        ari.train.skmeans.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$SKM.res$ari.train.tune})),na.rm=T)
        ari.test.skmeans.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$SKM.res$ari.test.tune})),na.rm=T)

        sensit3.train.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.skmeans,"[[",1))
        precis3.train.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.skmeans,"[[",2))
        sensit3.test.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.skmeans,"[[",1))
        precis3.test.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.skmeans,"[[",2))

        ari.train.PMBC.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$PMBC.res$ari.train.tune})),na.rm=T)
        ari.test.PMBC.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$PMBC.res$ari.test.tune})),na.rm=T)
        confmat3.train.PMBC <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$PMBC.res$cluster.train.tune,label.train.true[,x],C)})
        confmat3.test.PMBC <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$PMBC.res$cluster.test.tune,label.test.true[,x],C)})
        sensit3.train.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.PMBC,"[[",1))
        precis3.train.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.PMBC,"[[",2))
        sensit3.test.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.PMBC,"[[",1))
        precis3.test.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.PMBC,"[[",2))

        ari.train.PMBC.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$PMBC.res$ari.train.tune})),na.rm=T)
        ari.test.PMBC.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$PMBC.res$ari.test.tune})),na.rm=T)
        sensit3.train.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.PMBC,"[[",1))
        precis3.train.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.PMBC,"[[",2))
        sensit3.test.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.PMBC,"[[",1))
        precis3.test.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.PMBC,"[[",2))
        ...
        confmat3.train.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.train.num,label.train.true[,3],3)})
        confmat3.test.yujia <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$WJL.res$cluster.test.num,label.test.true[,3],3)})
        sensit3.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.yujia,"[[",1),na.rm=T)
        precis3.train.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.yujia,"[[",2),na.rm=T)
        sensit3.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.yujia,"[[",1),na.rm=T)
        precis3.test.yujia.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.yujia,"[[",2),na.rm=T)

        ari.train.yujia.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$WJL.res$ari.train.num})),na.rm=T)
        ari.test.yujia.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$WJL.res$ari.test.num})),na.rm=T)
        sensit3.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.yujia,"[[",1),na.rm=T)
        precis3.train.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.yujia,"[[",2),na.rm=T)
        sensit3.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.yujia,"[[",1),na.rm=T)
        precis3.test.yujia.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.yujia,"[[",2),na.rm=T)

        ari.train.Peng.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$GM.res$ari.train.num})),na.rm=T)
        ari.test.Peng.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$GM.res$ari.test.num})),na.rm=T)

        confmat3.train.Peng <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$GM.res$cluster.train.num,label.train.true[,3],3)})
        confmat3.test.Peng <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$GM.res$cluster.test.num,label.test.true[,3],3)})
        sensit3.train.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.Peng,"[[",1),na.rm=T)
        precis3.train.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.Peng,"[[",2),na.rm=T)
        sensit3.test.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.Peng,"[[",1),na.rm=T)
        precis3.test.Peng.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.Peng,"[[",2),na.rm=T)

        ari.train.Peng.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$GM.res$ari.train.num})),na.rm=T)
        ari.test.Peng.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$GM.res$ari.test.num})),na.rm=T)

        sensit3.train.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.Peng,"[[",1),na.rm=T)
        precis3.train.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.Peng,"[[",2),na.rm=T)
        sensit3.test.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.Peng,"[[",1),na.rm=T)
        precis3.test.Peng.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.Peng,"[[",2),na.rm=T)

        ari.train.skmeans.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$SKM.res$ari.train.num})),na.rm=T)
        ari.test.skmeans.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$SKM.res$ari.test.num})),na.rm=T)

        confmat3.train.skmeans <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$SKM.res$cluster.train.num,label.train.true[,3],3)})
        confmat3.test.skmeans <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$SKM.res$cluster.test.num,label.test.true[,3],3)})
        sensit3.train.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.skmeans,"[[",1),na.rm=T)
        precis3.train.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.skmeans,"[[",2),na.rm=T)
        sensit3.test.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.skmeans,"[[",1),na.rm=T)
        precis3.test.skmeans.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.skmeans,"[[",2),na.rm=T)

        ari.train.skmeans.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$SKM.res$ari.train.num})),na.rm=T)
        ari.test.skmeans.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$SKM.res$ari.test.num})),na.rm=T)

        sensit3.train.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.skmeans,"[[",1),na.rm=T)
        precis3.train.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.skmeans,"[[",2),na.rm=T)
        sensit3.test.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.skmeans,"[[",1),na.rm=T)
        precis3.test.skmeans.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.skmeans,"[[",2),na.rm=T)

        ari.train.PMBC.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$PMBC.res$ari.train.num})),na.rm=T)
        ari.test.PMBC.matrix.num[ind_c1,ind_mu]<-mean(unlist(lapply(RES,function(x){x$PMBC.res$ari.test.num})),na.rm=T)
        confmat3.train.PMBC <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$PMBC.res$cluster.train.num,label.train.true[,3],3)})
        confmat3.test.PMBC <- lapply(1:length(RES),function(x) {confmat(RES[[x]]$PMBC.res$cluster.test.num,label.test.true[,3],3)})
        sensit3.train.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.PMBC,"[[",1),na.rm=T)
        precis3.train.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.train.PMBC,"[[",2),na.rm=T)
        sensit3.test.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.PMBC,"[[",1),na.rm=T)
        precis3.test.PMBC.matrix.num[ind_c1,ind_mu] <- mean(sapply(confmat3.test.PMBC,"[[",2),na.rm=T)

        ari.train.PMBC.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$PMBC.res$ari.train.num})),na.rm=T)
        ari.test.PMBC.matrix.num.se[ind_c1,ind_mu]<-sd(unlist(lapply(RES,function(x){x$PMBC.res$ari.test.num})),na.rm=T)
        sensit3.train.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.PMBC,"[[",1),na.rm=T)
        precis3.train.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.train.PMBC,"[[",2),na.rm=T)
        sensit3.test.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.PMBC,"[[",1),na.rm=T)
        precis3.test.PMBC.matrix.num.se[ind_c1,ind_mu] <- sd(sapply(confmat3.test.PMBC,"[[",2),na.rm=T)

      }

    }

    ari.train1<-rbind(round(frac3_vector,2),
                      ari.train.yujia.matrix.num[ind_c1,],ari.train.Peng.matrix.num[ind_c1,],
                      ari.train.skmeans.matrix.num[ind_c1,],ari.train.PMBC.matrix.num[ind_c1,])
    ari.test1<-rbind(round(frac3_vector,2),
                     ari.test.yujia.matrix.num[ind_c1,],ari.test.Peng.matrix.num[ind_c1,],
                     ari.test.skmeans.matrix.num[ind_c1,],ari.test.PMBC.matrix.num[ind_c1,])
    sensit3.train<-rbind(round(frac3_vector,2),
                         sensit3.train.yujia.matrix.num[ind_c1,],sensit3.train.Peng.matrix.num[ind_c1,],
                         sensit3.train.skmeans.matrix.num[ind_c1,],sensit3.train.PMBC.matrix.num[ind_c1,])
    sensit3.test<-rbind(round(frac3_vector,2),
                        sensit3.test.yujia.matrix.num[ind_c1,],sensit3.test.Peng.matrix.num[ind_c1,],
                        sensit3.test.skmeans.matrix.num[ind_c1,],sensit3.test.PMBC.matrix.num[ind_c1,])
    precis3.train<-rbind(round(frac3_vector,2),
                         precis3.train.yujia.matrix.num[ind_c1,],precis3.train.Peng.matrix.num[ind_c1,],
                         precis3.train.skmeans.matrix.num[ind_c1,],precis3.train.PMBC.matrix.num[ind_c1,])
    precis3.test<-rbind(round(frac3_vector,2),
                        precis3.test.yujia.matrix.num[ind_c1,],precis3.test.Peng.matrix.num[ind_c1,],
                        precis3.test.skmeans.matrix.num[ind_c1,],precis3.test.PMBC.matrix.num[ind_c1,])

    ari.train1.se<-rbind(round(frac3_vector,2),
                         ari.train.yujia.matrix.num.se[ind_c1,],ari.train.Peng.matrix.num.se[ind_c1,],
                         ari.train.skmeans.matrix.num.se[ind_c1,],ari.train.PMBC.matrix.num.se[ind_c1,])
    ari.test1.se<-rbind(round(frac3_vector,2),
                        ari.test.yujia.matrix.num.se[ind_c1,],ari.test.Peng.matrix.num.se[ind_c1,],
                        ari.test.skmeans.matrix.num.se[ind_c1,],ari.test.PMBC.matrix.num.se[ind_c1,])
    sensit3.train.se<-rbind(round(frac3_vector,2),
                            sensit3.train.yujia.matrix.num.se[ind_c1,],sensit3.train.Peng.matrix.num.se[ind_c1,],
                            sensit3.train.skmeans.matrix.num.se[ind_c1,],sensit3.train.PMBC.matrix.num.se[ind_c1,])
    sensit3.test.se<-rbind(round(frac3_vector,2),
                           sensit3.test.yujia.matrix.num.se[ind_c1,],sensit3.test.Peng.matrix.num.se[ind_c1,],
                           sensit3.test.skmeans.matrix.num.se[ind_c1,],sensit3.test.PMBC.matrix.num.se[ind_c1,])
    precis3.train.se<-rbind(round(frac3_vector,2),
                            precis3.train.yujia.matrix.num.se[ind_c1,],precis3.train.Peng.matrix.num.se[ind_c1,],
                            precis3.train.skmeans.matrix.num.se[ind_c1,],precis3.train.PMBC.matrix.num.se[ind_c1,])
    precis3.test.se<-rbind(round(frac3_vector,2),
                           precis3.test.yujia.matrix.num.se[ind_c1,],precis3.test.Peng.matrix.num.se[ind_c1,],
                           precis3.test.skmeans.matrix.num.se[ind_c1,],precis3.test.PMBC.matrix.num.se[ind_c1,])


    rownames(ari.train1)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(ari.test1)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(sensit3.train)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(sensit3.test)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(precis3.train)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(precis3.test)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(ari.train1.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(ari.test1.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(sensit3.train.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(sensit3.test.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(precis3.train.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")
    rownames(precis3.test.se)<-c("fraction","ogClust_WJL","ogClust_GM","SKM","PMBC")


    ari.train1.se<-pivot_longer(data = as.data.frame(t(ari.train1.se)),cols=!fraction,names_to="method",values_to="value")
    ari.test1.se<-pivot_longer(data = as.data.frame(t(ari.test1.se)),cols=!fraction,names_to="method",values_to="value")
    sensit3.train.se<-pivot_longer(data = as.data.frame(t(sensit3.train.se)),cols=!fraction,names_to="method",values_to="value")
    sensit3.test.se<-pivot_longer(data = as.data.frame(t(sensit3.test.se)),cols=!fraction,names_to="method",values_to="value")
    precis3.train.se<-pivot_longer(data = as.data.frame(t(precis3.train.se)),cols=!fraction,names_to="method",values_to="value")
    precis3.test.se<-pivot_longer(data = as.data.frame(t(precis3.test.se)),cols=!fraction,names_to="method",values_to="value")

    ari.train1<-pivot_longer(data = as.data.frame(t(ari.train1)),cols=!fraction,names_to="method",values_to="value")
    ari.test1<-pivot_longer(data = as.data.frame(t(ari.test1)),cols=!fraction,names_to="method",values_to="value")
    sensit3.train<-pivot_longer(data = as.data.frame(t(sensit3.train)),cols=!fraction,names_to="method",values_to="value")
    sensit3.test<-pivot_longer(data = as.data.frame(t(sensit3.test)),cols=!fraction,names_to="method",values_to="value")
    precis3.train<-pivot_longer(data = as.data.frame(t(precis3.train)),cols=!fraction,names_to="method",values_to="value")
    precis3.test<-pivot_longer(data = as.data.frame(t(precis3.test)),cols=!fraction,names_to="method",values_to="value")

    ari.train1$se<-ari.train1.se$value/sqrt(50)
    ari.test1$se<-ari.test1.se$value/sqrt(50)
    sensit3.train$se<-sensit3.train.se$value/sqrt(50)
    sensit3.test$se<-sensit3.test.se$value/sqrt(50)
    precis3.train$se<-precis3.train.se$value/sqrt(50)
    precis3.test$se<-precis3.test.se$value/sqrt(50)


    Group<-c(rep("ARI.train",nrow(ari.train1)),rep("ARI.test",nrow(ari.train1)),
             rep("Sensitivity.train",nrow(ari.train1)),
             rep("Sensitivity.test",nrow(ari.train1)),
             rep("Precision.train",nrow(ari.train1)),
             rep("Precision.test",nrow(ari.train1)))
    data1<-rbind(ari.train1,ari.test1,sensit3.train,sensit3.test,
                 precis3.train,precis3.test)
    data1<-cbind(data1,Group)
    data1$Group<-factor(data1$Group,levels = c("ARI.train",
                                               "ARI.test",
                                               "Sensitivity.train","Precision.train",
                                               "Sensitivity.test","Precision.test"))
    data1$method=factor(data1$method,levels = c("ogClust_GM",  "ogClust_WJL", "SKM","PMBC"))

    data<-data1[which(data1$Group=="ARI.train"),]
    p<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=10, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+facet_wrap(~Group,scales = "free")+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20,face="bold"),
            strip.text.x = element_text(size = 20),legend.text = element_text(size=25),
            legend.title = element_text(size=25),
            legend.key.size = unit(1.5, 'cm'))+
      scale_x_continuous(breaks=round(frac3_vector,2),labels=fractions(frac3_vector))+xlab("Fraction of the smallest cluster")+
      scale_color_discrete(name = "method", labels = c(expression(ogClust[GM]), expression(ogClust[WJL]), expression(paste("sparse ",italic(K),"-means")),expression("PMBC")))+
      scale_shape_discrete(name = "method", labels = c(expression(ogClust[GM]), expression(ogClust[WJL]), expression(paste("sparse ",italic(K),"-means")),expression("PMBC")))+
      theme(legend.text.align = 0)
    p
    ...
    p1<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
      scale_x_continuous(breaks=round(frac3_vector,2),labels=fractions(frac3_vector))+xlab("Fraction of the smallest cluster")+ylab("ARI")
    p1

    data<-data1[which(data1$Group=="Sensitivity.train"),]
    p2<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
      scale_x_continuous(breaks=round(frac3_vector,2),labels=fractions(frac3_vector))+xlab("Fraction of the smallest cluster")+ylab("Sensitivity")
    p2

    data<-data1[which(data1$Group=="Precision.train"),]
    p3<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
      scale_x_continuous(breaks=round(frac3_vector,2),labels=fractions(frac3_vector))+xlab("Fraction of the smallest cluster")+ylab("Precision")
    p3


    data<-data1[which(data1$Group=="ARI.test"),]
    p5<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
    ...
    data<-data1[which(data1$Group=="Sensitivity.test"),]
    p6<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
    ...
    data<-data1[which(data1$Group=="Precision.test"),]
    p7<-ggplot(data)+aes(x=fraction,y=value,color=method,shape=method)+geom_point(size=2.5, position = position_dodge(0.1))+
      geom_errorbar(aes(ymin=value-se, ymax=value+se), size=0.6,width =.05,lwd=0.3,position=position_dodge(0.1))+
      theme(axis.text=element_text(size=20),axis.title=element_text(size=20),
            strip.text.x = element_text(size = 20),legend.position = 'none',
            plot.margin = unit(c(0.5,0.5,0.5,0.5), "cm"))+
    ...
    p1 <- annotate_figure(p1,top = text_grob(""))
    p2 <- annotate_figure(p2,top = text_grob(""))
    p3 <- annotate_figure(p3,top = text_grob(""))

    p5 <- annotate_figure(p5,top = text_grob(""))
    p6 <- annotate_figure(p6,top = text_grob(""))
    p7 <- annotate_figure(p7,top = text_grob(""))

    plot3<-ggarrange(p2,p3,p1, nrow=1)
    plot4<-ggarrange(p6,p7,p5,nrow=1)

    plot_combined<-grid.arrange(plot3,plot4,
                                layout_matrix = cbind(c(1,2)))
    ggsave(plot_combined,filename = figure.name, width = 50, height = 40, units = "cm")
  }
  print("WJL Sensitivity:")
  print(sensit3.train.yujia.matrix.num)
  print(sensit3.test.yujia.matrix.num)
  print("WJL Precision:")
  print(precis3.train.yujia.matrix.num)
  print(precis3.test.yujia.matrix.num)
  print("GM Sensitivity:")
  print(sensit3.train.Peng.matrix.num)
  print(sensit3.test.Peng.matrix.num)
  print("GM Precision:")
  print(precis3.train.Peng.matrix.num)
  print(precis3.test.Peng.matrix.num)
  print("SKM Sensitivity:")
  print(sensit3.train.skmeans.matrix.num)
  print(sensit3.test.skmeans.matrix.num)
  print("SKM Precision:")
  print(precis3.train.skmeans.matrix.num)
  print(precis3.test.skmeans.matrix.num)
  print("PMBC Sensitivity:")
  print(sensit3.train.PMBC.matrix.num)
  print(sensit3.test.PMBC.matrix.num)
  print("PMBC Precision:")
  print(precis3.train.PMBC.matrix.num)
  print(precis3.test.PMBC.matrix.num)
}

###Generate Supplement Figure VIIII
summaryUnbalance_gen(SC=FALSE,tune=TRUE,frac3_vector=c(1/3,1/4,1/5,1/7),c1_vector_all=c(2),C=3,mu=0.9,figure.name="unbalanced_mu=0.9.png")
